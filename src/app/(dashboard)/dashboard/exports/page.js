...
export default function ExportsPage() {
  ...
  const exportExcel = () => {
    ...
    const p = new URLSearchParams({ schoolId, ...(cls && { class: cls }), ...(section && { section }) });
      ...
      <div className="grid-2">
        <div className="card">
          ...
          <div style={{ display:'flex', gap:12, marginTop:8 }}>
            <button className="btn btn-success" style={{ flex:1 }} onClick={exportExcel}>
              <FiDownload size={15}/> Export Excel (.xlsx)
            </button>
            <button className="btn btn-primary" style={{ flex:1 }} onClick={exportPDF}>
              <FiFileText size={15}/> Export PDF (A4)
            </button>
          </div>
        </div>
        ...
        <div className="card">
          ...
          {[
            ['Excel Report', 'Full student data in spreadsheet format with Student ID, Name, Father Name, Class, Section, Blood Group, Mobile, Address, DOB.'],
            ['PDF Report', 'Print-ready A4 student list with school name header, class info and all student details in a clean table format.'],
          ].map(([title, desc]) => (
